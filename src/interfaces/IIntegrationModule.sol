...
interface IIntegrationModule {
    ...
    event BasePayReceived(address indexed from, address indexed to, uint256 amount, bytes32 reference);
    ...
    function registerBasePayment(address from, address to, uint256 amount, bytes32 reference, bytes calldata metadata) external returns (uint256);
